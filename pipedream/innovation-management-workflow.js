...
version: "1.0.0",
description: "Outcome-Driven Innovation (ODI) microservice with AI enhancement for market discovery and strategy formulation",
...
```
  ...
  const response = {
    ...
    market_opportunity_score: validatedStrategy.ai_insights.market_opportunity_score,
    next_actions: [
      'validate_with_stakeholders', 
      'conduct_customer_interviews',
      'begin_project_planning'
    ],
    estimated_market_size: validatedStrategy.market_analysis.total_addressable_market,
...
const discoveryResponse = await this.openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system", 
      content: "You are an expert in Outcome-Driven Innovation methodology and market analysis. Provide detailed, data-driven insights in valid JSON format."
    },
...
const validationResponse = await this.openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system",
      content: "You are a strategic business analyst specializing in market validation and opportunity prioritization."
    },
...
const preSellResponse = await this.openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system",
      content: "You are a product validation expert specializing in pre-sell testing and market validation experiments."
    },
...
const strategyResponse = await this.openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system",
      content: "You are a strategic innovation consultant expert in Outcome-Driven Innovation and business strategy formulation."
    },
...
const scoringResponse = await this.openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system",
      content: "You are a senior strategy analyst providing final validation and scoring for innovation strategies."
    },
...
const strategyDoc = {
status: strategy.validation_scores.strategic_recommendation === ‘pursue’ ? ‘validated’ : ‘draft’,
...
```
  ...
  processing_metadata: {
    ai_model_used: "gpt-4",
    processing_duration: Date.now() - (steps.trigger?.event?.timestamp || Date.now()),
    data_sources: Object.keys(strategy.data_sources || {}),
    confidence_level: strategy.ai_insights.confidence_score
  }
};

return await this.mongodb.collection('innovation_strategies').insertOne(strategyDoc);
```
...
return {
  pain_points: [
    { pain: "Manual data entry takes too long", frequency: 0.7, intensity: 8 },
    { pain: "Difficult to find information quickly", frequency: 0.6, intensity: 7 }
  ],
  feature_requests: [
...
return {
competitors: [
{ name: "Competitor A", market_share: 0.25, strengths: ["Brand"], weaknesses: ["Innovation"] },
{ name: "Competitor B", market_share: 0.18, strengths: ["Price"], weaknesses: ["UX"] }
],
market_gaps: [
...
strategic_goals: ["Market expansion", "Revenue growth", "Innovation leadership"]
};
},
